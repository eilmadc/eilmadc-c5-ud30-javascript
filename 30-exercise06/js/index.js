...
var letras = ['T', 'R', 'W', 'A', 'G', 'M', 'Y', 'F', 'P', 'D', 'X', 'B', 'N', 'J', 'Z', 'S', 'Q', 'V', 'H', 'L', 'C', 'K', 'E', 'R', 'T'];
checkDni = () => {
  ...
  if ((dni_user < 0) || (dni_user > 99999999)) { //numero dni incorrecto
    alert('El numero : ' + dni_user + ' no es válido');

  } else {
    var letraCalculada = calculaLetraDni(dni_user); //Calcula la letra del DNI
    console.log(letraCalculada);//Imprimo por consola del navegador la letra del DNI

    if (letraCalculada !== dni_letter) {//Comprueba la letra introducida igual a la letra calculada
      alert('La letra introducida es incorrecta');
    } else {
      alert('El DNI es correcto'); //DNI correcto
    }
  }
}
